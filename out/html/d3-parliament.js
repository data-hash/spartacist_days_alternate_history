    ...
    data.each(function (d) {
      ...
      (function () {
        ...
        seats.forEach(function (s) {
          ...
          s.party = party;
          s.data =
            typeof party.seats === "number" ? null : party.seats[seatIndex];
      ...
      var seatY = function (d) {
        return d.cartesian.y;
      };
      var seatColor = function (d) {
        return d.party.color;
      };
      var seatOutline = function (d) {
        return d.party.outline;
      };
      ...
      if (enter.fromCenter || enter.smallToBig) {
        var t = circlesEnter.transition().duration(function () {
      ...
      if (update.animate) {
        var circlesUpdate = circles.transition().duration(function () {
          return 1000 + Math.random() * 800;
        });
      } else {
      ...
      hitCirclesEnter.attr("class", function (d) {
        return "seat-hit " + ((d.party && d.party.id) || "");
      });
      hitCirclesEnter.attr("cx", seatX);
      hitCirclesEnter.attr("cy", seatY);
      hitCirclesEnter.attr("r", function (d) {
        return seatRadius(d) * 2;
      });
      ...
      hitCirclesEnter.on("mouseenter", function (e) {
        ...
        var partyData = d.filter(function (p) {
          return p.id === partyId;
        })[0];
        var nSeats = partyData ? partyData.seats : "?";
        var partyName = partyData
          ? partyData.legend || partyData.name || partyId.toUpperCase()
          : partyId.toUpperCase();
        var partyColor = partyData?.color || "#000";

        var img = tooltipList
          ? tooltipList.find(function (t) {
              return (
                t.searchString &&
                (t.searchString.toLowerCase() === partyId.toLowerCase() ||
                  t.searchString.toLowerCase() ===
                    partyId.toUpperCase().toLowerCase())
              );
            })
          : null;
        ...
        var imgHtml =
          img && img.img
            ? '<img src="' +
              img.img +
              '" style="height:36px; width:auto; margin-right:8px; object-fit:contain;">'
            : "";
        ...
        var img = tooltipList
          ? tooltipList.find(function (t) {
              return (
                t.searchString &&
                t.searchString.toUpperCase() === partyId.toUpperCase()
              );
            })
          : null;
        var tooltipContent = window.getDynamicTooltipContent(
          partyId.toUpperCase(),
          img,
        );
        tooltip
        ...
        tooltip.style("border-top-color", partyColor);
        tooltip.style("border-left-color", "transparent");
        tooltip.style("border-right-color", "transparent");
        tooltip.style("border-bottom-color", partyColor);
        document.documentElement.style.setProperty("--arrow_color", partyColor);
      });
